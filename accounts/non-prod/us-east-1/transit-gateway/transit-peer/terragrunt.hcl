...
dependency "data_vpc1" {
  config_path = "../../vpc/dev-qa-uat-chcs-vpc-non-prod"
}

dependency "data_vpc2" {
  config_path = "../../vpc/pci-chcs-vpc-non-prod"
}

dependency "data_vpc3" {
  config_path = "../../vpc/hippa-chcs-vpc-non-prod"
}
...
inputs = {
  name            = "TGW-PEER-PROD"
  description     = "My TGW shared from transit account to prod accounts"
  ...
  vpc_attachments = {
    ...
    DEV-QA-UAT-CHCS-VPC-NON-PROD = {
      tgw_id                                          = dependency.data_tgw.outputs.ec2_transit_gateway_id
      vpc_id                                          = dependency.data_vpc3.outputs.vpc_id
      subnet_ids                                      = dependency.data_vpc3.outputs.public_subnets_0
      dns_support                                     = true
    }
  }
