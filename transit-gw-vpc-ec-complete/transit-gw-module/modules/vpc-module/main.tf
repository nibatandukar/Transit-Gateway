...
resource "aws_route" "transit_gateway" {
 ...
 # route_table_id         = aws_route_table.private[0].id
  destination_cidr_block = "${element(var.tgw_cidr, count.index)}"
  transit_gateway_id         = var.transit_gateway_id
  depends_on = [var.transit_gateway_id]
  timeouts {
    create = "5m"
...
/*
locals {
  routes_with_destination_cidrblocks = setproduct(outputs.rtb_id_vpc1,  var.destination_cidr_block)
}
